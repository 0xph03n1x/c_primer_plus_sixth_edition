...
int main(void) {
  struct student class[CSIZE] = {{"Flip", "Snide"},
                                 {"Clare", "Voyans"},
                                 {"Bingo", "Higgs"},
                                 {"Fawn", "Hunter"}};
...
void find_means(struct student ar[], int lim) {
  ...
  for (i = 0; i < lim; i++) {
    for (sum = 0, j = 0; j < SCORES; j++) sum += ar[i].scores[j];
...
void show_class(const struct student ar[], int lim) {
  ...
  for (i = 0; i < lim; i++) {
    ...
    for (j = 0; j < SCORES; j++) printf("%6.1f ", ar[i].scores[j]);
    printf(" Average = %5.2f\n", ar[i].mean);
  }
}
...
void show_ave(const struct student ar[], int lim) {
  ...
  printf("\n%27s: ", "QUIZ AVERAGES");
  for (j = 0; j < SCORES; j++) {
    for (total = 0, i = 0; i < lim; i++) total += ar[i].scores[j];
    printf("%6.2f ", total / lim);
  }
  for (total = 0, i = 0; i < lim; i++) total += ar[i].mean;
  printf("     All = %5.2f\n", total / lim);
}
