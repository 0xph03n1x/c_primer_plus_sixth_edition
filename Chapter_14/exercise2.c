...
const struct month months_heap[12] = {
    {"January", "Jan", 31, 1},   {"February", "Feb", 29, 2},
    {"March", "Mar", 31, 3},     {"April", "Apr", 30, 4},
    {"May", "May", 31, 5},       {"June", "Jun", 30, 6},
    {"July", "Jul", 31, 7},      {"August", "Aug", 31, 8},
    {"September", "Sep", 30, 9}, {"October", "Oct", 31, 10},
...
int days_leap(int d, char* m, int y);
int main(void) {
  int day;
  char input[20];
  int year;
  ...
  while (scanf("%d", &day) == 1) {
    ...
    if (year % 4 == 0 && year % 400 != 0)
      daytotal = days_leap(day, input, year);
    else
...
int days(int d, char* m, int y) {
  ...
  if (isalpha(*m)) {
    m[0] = toupper(m[0]);
    for (i = 1; m[i] != '\0'; i++) m[i] = tolower(m[i]);
  }
  for (i = 0; i < 12; i++)
    if (strcmp(m, months[i].name) == 0 || strcmp(m, months[i].abbrev) == 0 ||
        atoi(m) == months[i].monumb) {
  ...
  if (mon_num == 0)
    ...
    for (i = 0; i < mon_num - 1; i++) total += months[i].days;
  total += d;
...
int days_leap(int d, char* m, int y) {
  ...
  if (mon_num == 0)
    ...
    for (i = 0; i < mon_num - 1; i++) total += months_heap[i].days;
  total += d;
