...
void show_empty(struct seat* ps) {
  ...
  for (i = 0; i < MAX; i++) {
    if (!ps[i].isassign) printf("%d ", ps[i].num);
  }
  printf("\n");
}
...
void assign_seat(struct seat* ps) {
  ...
  printf(
      "Please input the number of seat you want to book (1 to 12) "
      "in %s flight:\n",
      flight_num);
...
void delete_seat(struct seat* ps) {
  ...
  printf(
      "Please input the number of seat you want to delete (1 to 12) "
      "in %s flight:\n",
      flight_num);
