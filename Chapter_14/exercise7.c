...
char get_choice(void) {
  ...
  printf("q. quit.\n");
  while ((ch = getchar()) != 'a' && ch != 'c' && ch != 'd' && ch != 'p' &&
         ch != 'q') {
    while (getchar() != '\n') continue;
...
void change(struct book *ps) {
  ...
  if (count == 0)
    ...
    if (i == count) printf("Sorry, we can't find the book you input.\n");
  }
}
...
void del(struct book *ps) {
  ...
  if (count == 0) printf("Sorry, the list is empty.\n");
  printf("Please input the title of book you want to delete:\n");
  s_gets(name, MAXTITL);
  for (i = 0; i < count; i++) {
    if (!strcmp(name, ps[i].title)) {
      for (j = i + 1; j < count; j++) ps[j - 1] = ps[j];
      count--;
